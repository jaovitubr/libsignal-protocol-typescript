    ...
    var msrCrypto = function () {
            ...
            function bytesToString(textBytes) {
                ...
                for (var i = 0; i < textBytes.length;) {
                    ...
                    if (encodedChar < 128) {
                        ...
                        charCode = (encodedChar << 12) + (textBytes[i++] << 6) + textBytes[i++] - 0xe2080;
                    }
                    else {
                        charCode = (encodedChar << 18) + (textBytes[i++] << 12) + (textBytes[i++] << 6) + textBytes[i++] - 0x3c82080;
                    }
                    if (charCode > 0xffff) {
        ...
        var asn1 = (function () {
            ...
            function parse(bytes, force) {
                force = !!force;
                var type = asn1Types[bytes[0] & 0x1f], dataLen = bytes[1], i = 0, constructed = !!(bytes[0] & 0x20), remainder, child, header;
                if (dataLen & 0x80) {
            ...
            function toString(objTree, indent) {
                var output = new Array(indent + 1).join(' ') +
                    objTree.type +
                    ' (' +
                    objTree.length +
                    ') ' +
        ...
        function msrcryptoMath() {
            ...
            function fixedWindowRecode2(digits, windowSize) {
                var digLen = digits.length, bits = new Array(digLen * DIGIT_BITS), i = 0, j = 0, k = 0, r = 0, dig, result = new Array(Math.ceil((digLen * DIGIT_BITS) / windowSize));
            ...
            function fetchBits2(digits, startBit, count) {
                var startDigit = Math.floor(startBit / DIGIT_BITS), shiftRight = startBit % DIGIT_BITS;
                return ((digits[startDigit] >>> shiftRight) |
                    ((digits[startDigit + 1] << (DIGIT_BITS - shiftRight)) & (DIGIT_MASK >>> (DIGIT_BITS - count))));
            }
            ...
            function multiply(a, b, p) {
                ...
                var i, j, k, l, c, t1, t2, alen = a.length, blen = b.length, bi;
            ...
            function MontgomeryMultiplier(modulus, context) {
                ...
                function montgomeryMultiply(multiplicand, multiplier, result, ctx) {
                    ctx = ctx || this;
                    var m = ctx.m, s = m.length, mPrime = ctx.mPrime, m0 = ctx.m0, rightI, r0, q, i = 0, j, jm1, t1, t2, carry, rounds = 0;
                ...
                function optimalWindowSize(length) {
                    var i = 2, t1, t0, bits = length * DIGIT_BITS;
                ...
                function modExp(base, exponent, result, skipSideChannel) {
                    ...
                    var i, j, expBits = fixedWindowRecode2(exponent, windowBits).reverse(), partialResult = this.rModM.slice(0), baseTableLen = Math.pow(2, windowBits), bt = baseTable;
                ...
                function reduce(x, result) {
                    var k = this.m.length, q1, q2, q3, r1, r2, i, needSubtract, temp = [];
                    result = result || x;
                    q1 = x.slice(k - 1);
                    q2 = [];
        ...
        function MsrcryptoECC() {
            ...
            var EllipticCurveFp = function (p1, a1, b1, order, gx, gy) {
                ...
                return {
                    ...
                    allocatePointStorage: function () {
                        return EllipticCurvePointFp(this, false, cryptoMath.intToDigits(0, fieldStorageBitLength), cryptoMath.intToDigits(0, fieldStorageBitLength));
                    },
                    createPointAtInfinity: function () {
                        return EllipticCurvePointFp(this, true, cryptoMath.intToDigits(0, fieldStorageBitLength), cryptoMath.intToDigits(0, fieldStorageBitLength));
                    },
                };
            };
            ...
            var EllipticCurvePointFp = function (curve, isInfinity, x, y, z, isInMontgomeryForm) {
                ...
                function clone() {
                    var clonePoint = EllipticCurvePointFp(returnObj.curve, returnObj.isInfinity, createArray(returnObj.x), createArray(returnObj.y), returnObj.z ? createArray(returnObj.z) : null, returnObj.isInMontgomeryForm);
                ...
                createProperty(returnObj, 'isInMontgomeryForm', isInMontgomeryForm, function () {
                    return isInMontgomeryForm;
                }, function (val) {
                    isInMontgomeryForm = val;
                });
                createProperty(returnObj, 'isInfinity', isInfinity, function () {
                    return isInfinity;
                }, function (val) {
                    isInfinity = val;
                });
            ...
            var EllipticCurveOperatorFp = function (curve) {
                ...
                function mixedAdd(jacobianPoint, affinePoint, outputPoint) {
                    ...
                    if (jacobianPoint.isAffine) {
                        throw new Error('Given jacobianPoint was in Affine form. Use ConvertToJacobian()\
                     before calling DoubleJacobianAddAffinePoints().');
                    }
                    if (!affinePoint.isAffine) {
                        throw new Error('Given affinePoint was in Jacobian form. Use ConvertToAffine() before \
                     calling DoubleJacobianAddAffinePoints().');
                    }
                    if (outputPoint.isAffine) {
                        throw new Error('Given jacobianPoint was in Jacobian form. Use ConvertToJacobian() before \
                     calling DoubleJacobianAddAffinePoints().');
                    }
                ...
                function convertToJacobianForm(point) {
                    ...
                    var clonedDigits, i, zOne = point.isInMontgomeryForm ? onemontgomery : one;
                ...
                function convert_R1_to_R2(point) {
                    var curve = point.curve, modulus = curve.p, qq = {
                        x: point.x.slice(),
                        y: point.y.slice(),
                        z: point.z.slice(),
                        td: [],
                ...
                function convertTedToWeierstrass(tedPoint, wPoint) {
                    var a = tedPoint.curve.a.slice(), d = tedPoint.curve.d.slice(), p = tedPoint.curve.p, modMul = cryptoMath.modMul, modInv = cryptoMath.modInv;
                    temp1 = [5];
            ...
            var ModularSquareRootSolver = function (modulus) {
                ...
                var publicMethods = {
                    ...
                    jacobiSymbol: function (a) {
                        var modEightMask = 0x7, modFourMask = 0x3, aPrime, pPrime;
                        aPrime = a.slice();
                        pPrime = p.slice();
        ...
        var msrcryptoSha512 = (function () {
            ...
            function hashBlock(message, blockIndex, hv, k, w) {
                var t, i, blockBytes = 128, tah, tal, tbh, tbl, xh, xl, tc = [], td = [], te = [], index;
            ...
            var h384, h512, k512, der384, der512, der512_224, der512_256, upd = utils.unpackData;
            ...
            k512 = upd('QoovmNcoriJxN0SRI+9lzbXA+8/sTTsv6bXbpYGJ27w5VsJb80i1OFnxEfG2BdAZkj+CpK8ZT5urHF7' +
            ...
            return {
                aes: function (keyBytes) {
                    ...
                    var mixColumns = function (state) {
                        var a = state[0], b = state[1], c = state[2], d = state[3], e = state[4], f = state[5], g = state[6], h = state[7], i = state[8], j = state[9], k = state[10], l = state[11], m = state[12], n = state[13], o = state[14], p = state[15];
        ...
        var msrcryptoGcm = function (blockCipher) {
            ...
            function blockMultiplication(blockX, blockY) {
                var z = utils.getVector(16), v = blockY.slice(0), mask, j, i;
                for (i = 0; i < 128; i++) {
            ...
            function init(ivBytes, additionalBytes, tagLength) {
                ...
                if (mIvBytes.length === 12) {
                    ...
                    mJ0 = ghash(mH, mIvBytes.concat(utils.getVector(l + 8)).concat(numberTo8Bytes(mIvBytes.length * 8)));
                    mGHashState = utils.getVector(16);
                }
        ...
        function MsrcryptoEntropy() {
            ...
            var canCollect = (global && global.addEventListener) || (typeof document !== 'undefined' && document.attachEvent);
        ...
        var prime = (function () {
            ...
            function primeSieve(max) {
                var numbers = new Array(max + 1), results = [], i, j, limit = Math.sqrt(max) | 0;
                for (i = 3; i <= limit; i += 2) {
            ...
            function setupIncrementalTrialDivision(candidate) {
                var i, j, r, p, y, primeCount, len = candidate.length - 1, db = cryptoMath.DIGIT_BASE, h = candidate[len];
        ...
        var msrcryptoRsaBase = function (keyStruct) {
            var utils = msrcryptoUtilities, keyIsPrivate = keyStruct.hasOwnProperty('n') && keyStruct.hasOwnProperty('d'), keyIsCrt = keyStruct.hasOwnProperty('p') && keyStruct.hasOwnProperty('q'), modulusLength = keyStruct.n.length;
        ...
        var rsaShared = {
            mgf1: function (seedBytes, maskLen, hashFunction) {
                ...
                for (counter = 0; counter <= Math.floor(maskLen / hashByteLen); counter += 1) {
                    bytes = [(counter >>> 24) & 0xff, (counter >>> 16) & 0xff, (counter >>> 8) & 0xff, counter & 0xff];
        ...
        rsaMode.oaep = function (keyStruct, hashFunction) {
            var utils = msrcryptoUtilities, random = msrcryptoPseudoRandom, size = keyStruct.n.length;
        ...
        rsaMode.pkcs1Encrypt = function (keyStruct) {
            ...
            function unpad(paddedData) {
                var i, paddingIsValid = validatePadding(paddedData), startOfData = 0;
        ...
        rsaMode.pkcs1Sign = function (keyStruct, hashFunction) {
            var utils = msrcryptoUtilities, size = keyStruct.n.length;
            function emsa_pkcs1_v15_encode(messageBytes) {
                ...
                return [0x00, 0x01].concat(utils.getVector(size - tlen - 3, 0xff), [0], paddedData);
            }
            return {
        ...
        rsaMode.pss = function (keyStruct, hashFunction) {
            var utils = msrcryptoUtilities, random = msrcryptoPseudoRandom;
        ...
        var msrcryptoKdf = function (hashFunction) {
            ...
            function deriveKey(secretBytes, otherInfo, keyOutputLength) {
                var reps = Math.ceil(keyOutputLength / (hashFunction.hashLen / 8)), counter = 1, digest = secretBytes.concat(otherInfo), output = [];
        ...
        if (typeof operations !== 'undefined') {
            msrcryptoKdf.deriveKey = function (p) {
                ...
                var otherInfo = utils
                    ...
                    .concat(utils.toArray(alg.partyUInfo), utils.toArray(alg.partyVInfo), utils.toArray(alg.publicInfo), utils.toArray(alg.privateInfo));
            ...
            msrcryptoKdf.deriveBits = function (p) {
                ...
                msrcryptoKdfInstance = msrcryptoKdf(hashFunction);
                var alg = p.algorithm;
                var otherInfo = alg.algorithmId.concat(alg.partyUInfo, alg.partyVInfo, alg.publicInfo || [], alg.privateInfo || []);
        ...
        var msrcryptoPbkdf2 = (function () {
            function deriveBits(p) {
                ...
                function F(S, c, i) {
                    var result = [], u = S.concat([(i >>> 24) & 0xff, (i >>> 16) & 0xff, (i >>> 8) & 0xff, i & 0xff]);
                    for (var j = 0; j < c; j++) {
        ...
        msrcryptoSubtle = (function () {
            ...
            var workerManager = (function () {
                ...
                function useWebWorkers(enable) {
                    ...
                    function setWorkerStatus(e) {
                        ...
                        asyncMode = succeeded;
                        utils.consoleLog('web worker initialization ' +
                            (succeeded ? 'succeeded. Now using web workers.' : 'failed. running synchronously.' + (e.message || '')));
                        if (jobQueue.length > 0) {
            ...
            function buildParameterCollection(operationName, parameterSet) {
                ...
                }, operationParameterSet, expectedParam, actualParam, i;
                ...
                for (i = 0; i < operationParameterSet.length; i += 1) {
                    ...
                    //console.log('rolfe1:', actualParam)
                    //console.log('rolfe2:', expectedParam)
                    //console.log('rolfe3:', msrcryptoUtilities.getObjectType(actualParam))
                ...
                wrapKey: function (format, key, wrappingKey, wrappingKeyAlgorithm) {
                    ...
                    return new Promise(function (resolve, reject) {
                        ...
                        exportKey(format, key)
                            ...
                            return encrypt(wrappingKeyAlgorithm, wrappingKey, format === 'jwk' ? utils.stringToBytes(JSON.stringify(keyData, null, 0)) : keyData);
                        })
                ...
                unwrapKey: function (format, wrappedKey, unwrappingKey, unwrapAlgorithm, unwrappedKeyAlgorithm, extractable, keyUsages) {
                    ...
                    return new Promise(function (resolve, reject) {
                        ...
                        decrypt(unwrapAlgorithm, unwrappingKey, wrappedKey)
                            ...
                            return importKey(format, format === 'jwk' ? JSON.parse(utils.bytesToString(keyPlain)) : keyPlain, unwrappedKeyAlgorithm, extractable, keyUsages);
                        })
                            .then(function (key) {
                            resolve(key);
                        })['catch'](function (err) {
        ...
        var msrcryptoWrapKey = (function () {
            ...
            function wrapKey(params) {
                ...
                var encodedCiphertext = utils.toBase64(ciphertextPlusTag.slice(0, ciphertextPlusTag.length - tag.length), true);
                var jwe = {
                    recipients: [
                        {
            ...
            function unwrapKey(params) {
                ...
                var additionalData = keyDataJwk.recipients[0].header.concat('.', keyDataJwk.recipients[0].encrypted_key, '.', keyDataJwk.initialization_vector);
...
//# sourceMappingURL=msrcrypto.js.map